...
const expectations = {
    ...
    toBeNear: {
        test: (value, target, error = target * 0.01) =>
            Math.abs(value - target) <= error,
        message: (value, target, error = target * 0.01) =>
            `Expected ${value} to be near ${target} \u00B1 ${error}`,
    },
    toBeNull: {
                ...
                ex[name] = (...args) => {
                    ...
                    if (pass === false) {
                        bridge.dispatch(
                            "expect.fail",
                            new Error(
                                expectation.message(value, ...args)
                            )
                        )
                        return expector
                    }
